...
MAX_TENTATIVAS = 2
MAX_THREADS = 20
BUFFER_PARAMS = "buffer=4000000&timeout=8000&reconnect=1"
...
def processar_lista(texto):
    ...
    while i < len(linhas):
        if linhas[i].startswith("#EXTINF:") and i+1 < len(linhas) and linhas[i+1].startswith("http"):
            ...
            i += 1
    logging.info(f"{len(canais)} canais encontrados — testando...")

    with ThreadPoolExecutor(max_workers=MAX_THREADS) as ex:
        res = list(ex.map(testar_canal, canais))
    bons = sorted([c for c in res if c["ok"]], key=lambda x: x["velocidade"])
    logging.info(f"{len(bons)} canais rápidos e funcionando")
...
class Servidor(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/lista.m3u":
            ...
            self.send_response(200)
            self.end_headers()
            self.wfile.write(b"OK - /lista.m3u")
...
def atualizar_loop():
    ...
    while True:
        try:
            ...
            logging.info("✅ Lista atualizada e otimizada")
        except Exception as e:
